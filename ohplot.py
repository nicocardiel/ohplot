...
def ohplot(
    ...
    xwave_oliva, sp_oh_oliva = convolve_oh_lines(
        ohlines_oliva_wave, ohlines_oliva_flux, sigma,
        crpix1, crval1, cdelt1, naxis1
    )
    ...
    xwave_iraf, sp_oh_iraf = convolve_oh_lines(
        ohlines_iraf_wave, ohlines_iraf_flux, sigma,
        crpix1, crval1, cdelt1, naxis1
    )
    ...
    if emlines:
        ...
        for wavedum in emission_lines:
            ax.axvline(wavedum * (1 + redshift), color='k',
                       linestyle='--')
    ...
    if filename is not None:
        ...
        ax.text(1.0, 1.02, 'z={:8.6f}'.format(redshift), fontsize=10,
...
def main(args=None):
    ...
    if args.ascii is not None:
        ascii_filename = args.ascii.name
    else:
        ascii_filename = None
